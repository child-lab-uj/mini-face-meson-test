...
class FaceAnalyser {
  ...
  private:
    ...
    // A utility function for keeping track of approximate running medians used for AU and emotion inference using a set
    // of histograms (the histograms are evenly spaced from min_val to max_val) Descriptor has to be a row vector
    // TODO this duplicates some other code
    void UpdateRunningMedian(
    ...
    // The AUs predicted by the model are not always 0 calibrated to a person. That is they don't always predict 0 for a
    // neutral expression Keeping track of the predictions we can correct for this, by assuming that at least "ratio" of
    // frames are neutral and subtract that value of prediction, only perform the correction after min_frames
