...
class Patch_experts {
  ...
  public:
    // The collection of SVR patch experts (for intensity/grayscale images), the experts are laid out
    // scale->view->landmark
    ...
    // The computation also requires the current landmark locations to compute response around, the PDM corresponding to
    // the desired model, and the parameters describing its instance Also need to provide the size of the area of
    // interest and the desired scale of analysis
    void Response(
  ...
  private:
    ...
    std::vector<int>
    Collect_visible_landmarks(std::vector<std::vector<cv::Mat_<int>>> visibilities, int scale, int view_id, int n);
};
