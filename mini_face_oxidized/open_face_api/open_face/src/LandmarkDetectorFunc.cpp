...
// Getting a head pose estimate from the currently detected landmarks, with appropriate correction due to the PDM
// assuming an orthographic camera which is only correct close to the centre of the image This method returns a
// corrected pose estimate with respect to world coordinates with camera at origin (0,0,0) The format returned is [Tx,
// Ty, Tz, Eul_x, Eul_y, Eul_z]
...
// Getting a head pose estimate from the currently detected landmarks, with appropriate correction due to perspective
// projection This method returns a corrected pose estimate with respect to a point camera (NOTE not the world
// coordinates), which is useful to find out if the person is looking at a camera The format returned is [Tx, Ty, Tz,
// Eul_x, Eul_y, Eul_z]
cv::Vec6f LandmarkDetector::GetPoseWRTCamera(const CLNF &clnf_model, float fx, float fy, float cx, float cy) {
    if (!clnf_model.detected_landmarks.empty() && clnf_model.params_global[0] != 0) {
        ...
        // Here we correct for the camera orientation, for this need to determine the angle the camera makes with the
        // head pose
        ...
        return cv::Vec6f(
            vec_trans[0], vec_trans[1], vec_trans[2], euler_corrected[0], euler_corrected[1], euler_corrected[2]
        );
    } else {
        return cv::Vec6f(0, 0, 0, 0, 0, 0);
    }
...
void CorrectGlobalParametersVideo(
    ...
    cv::Rect roi(
        init_box.x - init_box.width / 2, init_box.y - init_box.height / 2, init_box.width * 2, init_box.height * 2
    );
...
bool LandmarkDetector::DetectLandmarksInVideo(
    ...
    // This is used for both detection (if it the tracking has not been initialised yet) or if the tracking failed
    // (however we do this every n frames, for speed) This also has the effect of an attempt to reinitialise just after
    ...
    if ((!clnf_model.tracking_initialised && (clnf_model.failures_in_a_row + 1) % (params.reinit_video_every * 6) == 0
        ...
        if (params.curr_face_detector == FaceModelParameters::HOG_SVM_DETECTOR) {
            ...
            face_detection_success = LandmarkDetector::DetectSingleFaceHOG(
                bounding_box, grayscale_image, clnf_model.face_detector_HOG, confidence, preference_det
            );
            ...
            face_detection_success = LandmarkDetector::DetectSingleFace(
                bounding_box, grayscale_image, clnf_model.face_detector_HAAR, preference_det
            );
            ...
            face_detection_success = LandmarkDetector::DetectSingleFaceMTCNN(
                bounding_box, rgb_image, clnf_model.face_detector_MTCNN, confidence, preference_det
            );
        }
        ...
        if (face_detection_success) {
            ...
            // TODO rem (should the multi-hyp version be only for CEN and not CLNF?), otherwise poss too slow, and poss
            // not accurate
            ...
            bool landmark_detection_success =
                DetectLandmarksInImage(rgb_image, bounding_box, clnf_model, params, grayscale_image);
            params.multi_view = false;

            // If landmark reinitialisation unsucessful continue from previous estimates
            // if it's initial detection however, do not care if it was successful as the validator might be wrong, so
            // continue trackig regardless
            if (!initial_detection && !landmark_detection_success) {
                ...
                clnf_model.pdm.CalcShape2D(
                    clnf_model.detected_landmarks, clnf_model.params_local, clnf_model.params_global
                );
...
bool LandmarkDetector::DetectLandmarksInVideo(
    ...
    if (bounding_box.width > 0) {
        // calculate the local and global parameters from the generated 2D shape (mapping from the 2D to 3D because
        // camera params are unknown)
        clnf_model.params_local.setTo(0);
        clnf_model.pdm.CalcParams(clnf_model.params_global, bounding_box, clnf_model.params_local);
...
//================================================================================================================
// Landmark detection in image, need to provide an image and optionally CLNF model together with parameters (default
// values work well) Optionally can provide a bounding box in which detection is performed (this is useful if multiple
// faces are to be detected in images)
//================================================================================================================
...
bool DetectLandmarksInImageMultiHypBasic(
    ...
    for (size_t hypothesis = 0; hypothesis < rotation_hypotheses.size(); ++hypothesis) {
        ...
        // calculate the local and global parameters from the generated 2D shape (mapping from the 2D to 3D because
        // camera params are unknown)
        clnf_model.pdm.CalcParams(
            clnf_model.params_global, bounding_box, clnf_model.params_local, rotation_hypotheses[hypothesis]
        );
...
bool DetectLandmarksInImageMultiHypEarlyTerm(
    ...
    for (size_t hypothesis = 0; hypothesis < rotation_hypotheses.size(); ++hypothesis) {
        ...
        float lhood = clnf_model.model_likelihood * clnf_model.patch_experts.early_term_weights[clnf_model.view_used] +
...
bool LandmarkDetector::DetectLandmarksInImage(
    ...
    if (clnf_model.patch_experts.early_term_biases.size() == 0) {
        success =
            DetectLandmarksInImageMultiHypBasic(grayscale_image, rotation_hypotheses, bounding_box, clnf_model, params);
    } else {
