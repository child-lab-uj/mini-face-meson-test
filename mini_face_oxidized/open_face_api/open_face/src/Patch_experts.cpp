...
Patch_experts::Patch_experts(const Patch_experts &other) :
    patch_scaling(other.patch_scaling), centers(other.centers), svr_expert_intensity(other.svr_expert_intensity),
    ...
    early_term_cutoffs(other.early_term_cutoffs), mirror_inds(other.mirror_inds), mirror_views(other.mirror_views) {
...
// Returns indices to landmarks that need to have patch responses computed (omits mirrored frontal landmarks for CEN as
// they will be computed together with their mirrored pair)
        ...
        if (visibilities[scale][view_id].rows == n) {
            if (visibilities[scale][view_id].at<int>(i, 0) != 0) {
                ...
                if (!cen_expert_intensity.empty()) {
                    ...
                    if (view_id == 0) {
                        // If the patch expert does not have values, means it's a mirrored version and will be done in
                        // another part of a loop
...
// The computation also requires the current landmark locations to compute response around, the PDM corresponding to the
// desired model, and the parameters describing its instance Also need to provide the size of the area of interest and
...
void Patch_experts::Response(
    ...
    parallel_for_(cv::Range(0, vis_lmk.size()), [&](const cv::Range &range) {
        for (int i = range.start; i < range.end; i++) {
            ...
            cv::Mat sim =
                (cv::Mat_<float>(2, 3) << a1,
                 -b1,
                 landmark_locations.at<float>(ind, 0) - a1 * (area_of_interest_width - 1.0f) / 2.0f +
                     b1 * (area_of_interest_width - 1.0f) / 2.0f,
                 b1,
                 a1,
                 landmark_locations.at<float>(ind + n, 0) - a1 * (area_of_interest_width - 1.0f) / 2.0f -
                     b1 * (area_of_interest_width - 1.0f) / 2.0f);
            ...
            cv::warpAffine(
                grayscale_image, area_of_interest, sim, area_of_interest.size(), cv::WARP_INVERSE_MAP + cv::INTER_LINEAR
            );

            // Get intensity response either from the SVR, CCNF, or CEN patch experts (prefer CEN as they are the most
            // accurate so far)
            if (!cen_expert_intensity.empty()) {
                ...
                if (view_id == 0) {
                    ...
                    if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
                        ...
                        if (mirror_id == ind) {
                            ...
                            cen_expert_intensity[scale][view_id][ind].ResponseSparse(
                                ...
                                area_of_interest_r,
                                patch_expert_responses[ind],
                                patch_expert_responses[mirror_id],
                                interp_mat,
                                prealloc_mat,
                                prealloc_mat_right
                            );
                    ...
                    if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
                        cv::Mat_<float> empty(0, 0, 0.0f);
                        cen_expert_intensity[scale][view_id][ind].ResponseSparse(
                            area_of_interest, empty, patch_expert_responses[ind], empty, interp_mat, prealloc_mat, empty
                        );
                        ...
                        cv::Mat_<float> empty(0, 0, 0.0f);
                        cen_expert_intensity[scale][mirror_views.at<int>(view_id)][mirror_inds.at<int>(ind)]
                            .ResponseSparse(
                                empty,
                                area_of_interest,
                                empty,
...
bool Patch_experts::Read(
    ...
    for (int scale = 0; scale < num_intensity_svr; ++scale) {
        ...
        bool success_read = Read_SVR_patch_experts(
            location, centers[scale], visibilities[scale], svr_expert_intensity[scale], patch_scaling[scale]
        );
        if (!success_read) {
            return false;
        }
    ...
    for (int scale = 0; scale < num_intensity_ccnf; ++scale) {
        ...
        bool success_read = Read_CCNF_patch_experts(
            location, centers[scale], visibilities[scale], ccnf_expert_intensity[scale], patch_scaling[scale]
        );
    ...
    for (int scale = 0; scale < num_intensity_cen; ++scale) {
        ...
        bool success_read = Read_CEN_patch_experts(
            location, centers[scale], visibilities[scale], cen_expert_intensity[scale], patch_scaling[scale]
        );
        if (!success_read) {
            return false;
        }
...
bool Patch_experts::Read_CCNF_patch_experts(
    ...
    if (patchesFile.is_open()) {
        ...
        // Read the possible SigmaInvs (without beta), this will be followed by patch reading (this assumes all of them
        // have the same type, and number of betas)
        int num_win_sizes;
        int num_sigma_comp;
...
bool Patch_experts::Read_CEN_patch_experts(
    ...
    if (patchesFile.is_open()) {
        ...
        std::cout << "Could not find CEN patch experts, for instructions of how to download them, see "
