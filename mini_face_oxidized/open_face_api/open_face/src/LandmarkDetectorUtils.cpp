...
void matchTemplate_m(
    ...
    int numType = method == cv::TM_CCORR || method == cv::TM_CCORR_NORMED     ? 0
                  : method == cv::TM_CCOEFF || method == cv::TM_CCOEFF_NORMED ? 1
                                                                              : 2;
    bool isNormed = method == cv::TM_CCORR_NORMED || method == cv::TM_SQDIFF_NORMED || method == cv::TM_CCOEFF_NORMED;
...
// Useful utility for grabing a bounding box around a set of 2D landmarks (as a 1D 2n x 1 vector of xs followed by
// doubles or as an n x 2 vector)
...
std::vector<cv::Point2f> CalculateVisibleLandmarks(const CLNF &clnf_model) {
    ...
    if (clnf_model.detection_success) {
        ...
        // Because we only draw visible points, need to find which points patch experts consider visible at a certain
        // orientation
...
bool DetectFaces(
    ...
    for (size_t face = 0; face < face_detections.size(); ++face) {
        ...
        // CLNF detector expects the bounding box to encompass from eyebrow to chin in y, and from cheeck outline to
        // cheeck outline in x, so we need to compensate

        // The scalings were learned using the Face Detections on LFPW, Helen, AFW and iBUG datasets, using ground truth
        // and detections from openCV
        ...
        if (min_width != -1) {
            if (region.width < min_width || region.x < ((float) intensity.cols) * roi.x ||
                region.y < ((float) intensity.cols) * roi.y ||
                region.x + region.width > ((float) intensity.cols) * (roi.x + roi.width) ||
                region.y + region.height > ((float) intensity.rows) * (roi.y + roi.height))
                continue;
        }
...
bool DetectFacesHOG(
    ...
    cv::resize(
        intensity, upsampled_intensity, cv::Size((int) (intensity.cols * scaling), (int) (intensity.rows * scaling))
    );
    ...
    // Convert from int bounding box do a double one with corrections
    for (size_t face = 0; face < face_detections.size(); ++face) {
        // CLNF expects the bounding box to encompass from eyebrow to chin in y, and from cheeck outline to cheeck
        // outline in x, so we need to compensate
        ...
        // The scalings were learned using the Face Detections on LFPW and Helen using ground truth and detections from
        // the HOG detector
        if (min_width != -1) {
            if (region.width < min_width || region.x < ((float) intensity.cols) * roi.x ||
                region.y < ((float) intensity.cols) * roi.y ||
...
bool DetectSingleFaceHOG(
    ...
    std::vector<float> confidences;
    bool detect_success =
        LandmarkDetector::DetectFacesHOG(face_detections, intensity_img, detector, confidences, min_width, roi);
