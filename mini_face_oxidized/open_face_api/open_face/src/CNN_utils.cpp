...
void im2col_multimap(
    const std::vector<cv::Mat_<float>> &inputs,
    const unsigned int width,
    const unsigned int height,
    cv::Mat_<float> &output
) {
...
void convolution_direct_blas(
    ...
    // Instead of re-allocating data use the first rows of already allocated data and re-allocate only if not enough
    // rows are present, this is what makes this non thread safe, as same memory would be used
    ...
    // cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, weight_t.cols, yB * xB, pre_alloc_im2col.cols, 1, m2,
    // weight_t.cols, m1, pre_alloc_im2col.cols, 0.0, m3, weight_t.cols);
    float alpha = 1.0f;
    float beta = 0.0f;
