...
// Copy constructor	(do not perform a deep copy of data as it is very large, also there is no real need to stor the
// copies
CEN_patch_expert::CEN_patch_expert(const CEN_patch_expert &other) :
    confidence(other.confidence), width_support(other.width_support), height_support(other.height_support) {
...
void CEN_patch_expert::Response(const cv::Mat_<float> &area_of_interest, cv::Mat_<float> &response) {
    ...
    for (size_t layer = 0; layer < activation_function.size(); ++layer) {
        ...
        // We are performing response = weights[layers] * response(t), but in OpenBLAS as that is significantly quicker
        // than OpenCV
        ...
        // cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, resp.cols, weight.rows, weight.cols, 1, m1, resp.cols,
        // m2, weight.cols, 0.0, m3, resp.cols);
...
// Perform im2col, while at the same time doing contrast normalization and adding a bias term (also skip every other
// region)
void im2colBiasSparseContrastNorm(
    const cv::Mat_<float> &input,
    const unsigned int width,
    const unsigned int height,
    cv::Mat_<float> &output
...
void LandmarkDetector::interpolationMatrix(
...
void CEN_patch_expert::ResponseInternal(cv::Mat_<float> &response) {
    for (size_t layer = 0; layer < activation_function.size(); ++layer) {
        ...
        // We are performing response = weights[layers] * response, but in OpenBLAS as that is significantly quicker
        // than OpenCV
        cv::Mat_<float> resp = response;
        float *m1 = (float *) resp.data;
