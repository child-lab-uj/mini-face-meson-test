...
CCNF_neuron::CCNF_neuron(const CCNF_neuron &other) : weights(other.weights.clone()) {
    ...
    for (std::map<int, cv::Mat_<double>>::const_iterator it = other.weights_dfts.begin();
         it != other.weights_dfts.end();
         it++) {
...
CCNF_patch_expert::CCNF_patch_expert(const CCNF_patch_expert &other) :
    neurons(other.neurons), window_sizes(other.window_sizes), betas(other.betas) {
...
//===========================================================================
void CCNF_neuron::Response(
    const cv::Mat_<float> &im,
    ...
    if (neuron_type == 3) {
        ...
        matchTemplate_m(
            I, im_dft, integral_img, integral_img_sq, weights, weights_dfts, resp, cv::TM_CCOEFF
        ); // the linear multiplication, efficient calc of response
    } else {
        matchTemplate_m(
            I, im_dft, integral_img, integral_img_sq, weights, weights_dfts, resp, cv::TM_CCOEFF_NORMED
        ); // the linear multiplication, efficient calc of response
    }
...
void CCNF_patch_expert::Response(const cv::Mat_<float> &area_of_interest, cv::Mat_<float> &response) {
    ...
    // the placeholder for the DFT of the image, the integral image, and squared integral image so they don't get
    // recalculated for every response
    ...
    for (size_t i = 0; i < neurons.size(); i++) {
        ...
        if (neurons[i].alpha > 1e-4) {
            ...
            neurons[i].Response(
                area_of_interest, area_of_interest_dft, integral_image, integral_image_sq, neuron_response
            );
...
void CCNF_patch_expert::ResponseOpenBlas(
    const cv::Mat_<float> &area_of_interest,
    ...
    sgemm_(
        N,
        N,
        &normalized_input.cols,
        &weight_matrix.rows,
        &weight_matrix.cols,
        &alpha1,
        (float *) normalized_input.data,
        &normalized_input.cols,
        (float *) weight_matrix.data,
        &weight_matrix.cols,
        &beta1,
        (float *) neuron_resp_full.data,
        &normalized_input.cols
    );
    ...
    sgemm_(
        ...
        N,
        &resp_vec_f.cols,
        &Sigmas[s_to_use].rows,
        &Sigmas[s_to_use].cols,
        &alpha1,
        (float *) resp_vec_f.data,
        &resp_vec_f.cols,
        (float *) Sigmas[s_to_use].data,
        &Sigmas[s_to_use].cols,
        &beta1,
        (float *) out.data,
        &resp_vec_f.cols
