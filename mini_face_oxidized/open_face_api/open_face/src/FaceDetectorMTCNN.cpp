...
// Instead of including cblas.h (the definitions from OpenBLAS and other BLAS libraries differ, declare the required
// OpenBLAS functionality here)
#ifdef __cplusplus
extern "C" {
...
FaceDetectorMTCNN::FaceDetectorMTCNN(const std::string &location) { this->Read(location); }
// Copy constructor
...
std::vector<cv::Mat_<float>> CNN::Inference(const cv::Mat &input_img, bool direct, bool thread_safe) {
    ...
    for (size_t layer = 0; layer < cnn_layer_types.size(); ++layer) {
        ...
        if (layer_type == 0) {
            ...
            // Either perform direct convolution through matrix multiplication or use an FFT optimized version, which
            // one is optimal depends on the kernel and input sizes
            if (direct) {
                if (thread_safe) {
                    ...
                    convolution_direct_blas(
                        outputs,
                        input_maps,
                        cnn_convolutional_layers_weights[cnn_layer],
            ...
            // cnn_convolutional_layers_bias[cnn_layer], cnn_convolutional_layers_dft[cnn_layer]);
        ...
        for (int layer = 0; layer < network_depth; ++layer) {
            ...
            if (layer_type == 0) {
                ...
                cv::Mat_<float> weight_matrix(
                    num_in_maps * kernels_rearr[0][0].rows * kernels_rearr[0][0].cols, num_kernels
                );
                for (int k = 0; k < num_kernels; ++k) {
                    for (int i = 0; i < num_in_maps; ++i) {
                        ...
                        k_flat.copyTo(weight_matrix(cv::Rect(
                            k,
                            i * kernels_rearr[0][0].rows * kernels_rearr[0][0].cols,
                            1,
...
std::vector<int> non_maximum_supression(
    const std::vector<cv::Rect_<float>> &original_bb,
    const std::vector<float> &scores,
    float thresh,
    bool minimum
) {

    // Sort the input bounding boxes by the detection score, using the nice trick of multimap always being sorted
    // internally
...
// Use the heatmap generated by PNet to generate bounding boxes in the original image space, also generate the
// correction values and scores of the bounding boxes as well
void generate_bounding_boxes(
    std::vector<cv::Rect_<float>> &o_bounding_boxes,
...
void apply_correction(
    std::vector<cv::Rect_<float>> &total_bboxes,
...
bool FaceDetectorMTCNN::DetectFaces(
    std::vector<cv::Rect_<float>> &o_regions,
    const cv::Mat &img_in,
    ...
    for (int i = 0; i < num_scales; ++i) {
        ...
        generate_bounding_boxes(
            proposal_boxes, scores, proposal_corrections, prob_heatmap, corrections_heatmap, scale, t1, face_support
        );
    ...
    for (int i = 0; i < num_scales; ++i) {
        std::vector<int> to_keep =
            non_maximum_supression(proposal_boxes_cross_scale[i], scores_cross_scale[i], 0.5, false);
        select_subset(
            to_keep, proposal_boxes_cross_scale[i], scores_cross_scale[i], proposal_corrections_cross_scale[i]
        );

        proposal_boxes_all.insert(
            proposal_boxes_all.end(), proposal_boxes_cross_scale[i].begin(), proposal_boxes_cross_scale[i].end()
        );
        scores_all.insert(scores_all.end(), scores_cross_scale[i].begin(), scores_cross_scale[i].end());
        proposal_corrections_all.insert(
            proposal_corrections_all.end(),
    ...
    for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
        ...
        int end_x_out =
            cv::min(width_target - (proposal_boxes_all[k].x + proposal_boxes_all[k].width - width_orig), width_target);
        int end_y_out = cv::min(
            height_target - (proposal_boxes_all[k].y + proposal_boxes_all[k].height - height_orig), height_target
        );

        cv::Mat tmp(height_target, width_target, CV_32FC3, cv::Scalar(0.0f, 0.0f, 0.0f));

        img_float(cv::Rect(start_x_in, start_y_in, end_x_in - start_x_in, end_y_in - start_y_in))
            .copyTo(tmp(cv::Rect(start_x_out, start_y_out, end_x_out - start_x_out, end_y_out - start_y_out)));
    ...
    for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
        ...
        o_regions.push_back(cv::Rect_<float>(
            proposal_boxes_all[k].x, proposal_boxes_all[k].y, proposal_boxes_all[k].width, proposal_boxes_all[k].height
        ));
