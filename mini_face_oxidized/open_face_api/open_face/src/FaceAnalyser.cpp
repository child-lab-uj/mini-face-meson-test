...
void FaceAnalyser::GetLatestAlignedFace(cv::Mat &image) { image = this->aligned_face_for_output.clone(); }
...
void FaceAnalyser::PredictStaticAUsAndComputeFeatures(const cv::Mat &frame, const cv::Mat_<float> &detected_landmarks) {
    ...
    // cv::Mat_<uchar> aligned_face_cols(1, aligned_face_for_au.cols * aligned_face_for_au.rows *
    // aligned_face_for_au.channels(), aligned_face_for_au.data, 1); cv::Mat_<double> aligned_face_cols_double;
...
void FaceAnalyser::AddNextFrame(
    const cv::Mat &frame,
    ...
    if (frames_tracking % 2 == 1) {
        UpdateRunningMedian(
            this->hog_desc_hist[orientation_to_use],
    ...
    if (online) {
        AU_predictions_reg_corrected =
            CorrectOnlineAUs(AU_predictions_reg, orientation_to_use, true, false, success, true);
...
void FaceAnalyser::GetGeomDescriptor(cv::Mat_<double> &geom_desc) { geom_desc = this->geom_descriptor_frame.clone(); }
    ...
    if (!postprocessed) {
        ...
        while (all_ind < all_frames_size && success_ind < max_init_frames) {
            ...
            if (valid_preds[all_ind]) {
                ...
                for (size_t au = 0; au < AU_predictions_class.size(); ++au) {
                    // Find the appropriate AU (if not found add it)
                    AU_predictions_class_all_hist[AU_predictions_class[au].first][all_ind] =
...
void FaceAnalyser::ExtractAllPredictionsOfflineClass(
    ...
    for (auto au_iter = AU_predictions_class_all_hist.begin(); au_iter != AU_predictions_class_all_hist.end();
         ++au_iter) {
        std::string au_name = au_iter->first;
        std::vector<double> au_vals = au_iter->second;
...
void FaceAnalyser::Reset() {
    ...
    for (size_t i = 0; i < hog_desc_hist.size(); ++i) {
        ...
        this->au_prediction_correction_histogram[i] = cv::Mat_<int>(
            au_prediction_correction_histogram[i].rows, au_prediction_correction_histogram[i].cols, (int) 0
        );
    }
...
void FaceAnalyser::ExtractMedian(
    cv::Mat_<int> &histogram,
    int hist_count,
    cv::Mat_<double> &median,
    int num_bins,
...
std::vector<std::pair<std::string, double>> FaceAnalyser::PredictCurrentAUs(int view) {
    ...
    if (!hog_desc_frame.empty()) {
        ...
        AU_SVR_static_appearance_lin_regressors.Predict(
            svr_lin_stat_preds, svr_lin_stat_aus, hog_desc_frame, geom_descriptor_frame
        );
...
std::vector<std::pair<std::string, double>> FaceAnalyser::CorrectOnlineAUs(
    ...
    if (update_track) {
        UpdatePredictionTrack(
            au_prediction_correction_histogram[view],
            au_prediction_correction_count[view],
            correction,
            predictions,
            0.10,
            200,
...
std::vector<std::pair<std::string, double>> FaceAnalyser::PredictCurrentAUsClass(int view) {
    ...
    if (!hog_desc_frame.empty()) {
        ...
        AU_SVM_static_appearance_lin.Predict(
            svm_lin_stat_preds, svm_lin_stat_aus, hog_desc_frame, geom_descriptor_frame
        );
        ...
        AU_SVM_dynamic_appearance_lin.Predict(
            svm_lin_dyn_preds,
            svm_lin_dyn_aus,
            hog_desc_frame,
            geom_descriptor_frame,
...
std::vector<std::pair<std::string, double>> FaceAnalyser::GetCurrentAUsClass() const { return AU_predictions_class; }

std::vector<std::pair<std::string, double>> FaceAnalyser::GetCurrentAUsReg() const { return AU_predictions_reg; }
...
void FaceAnalyser::ReadAU(std::string au_model_location) {
    ...
    if (!locations.is_open()) {
        std::cout << "Couldn't open the AU prediction files at: " << au_model_location.c_str() << " aborting"
                  << std::endl;
        std::cout.flush();
        return;
    }
...
void FaceAnalyser::GetSampleHist(
    cv::Mat_<int> &prediction_corr_histogram,
    int prediction_correction_count,
