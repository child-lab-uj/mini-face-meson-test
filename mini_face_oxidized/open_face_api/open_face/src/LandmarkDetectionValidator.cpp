...
DetectionValidator::DetectionValidator(const DetectionValidator &other) :
    orientations(other.orientations), paws(other.paws), cnn_subsampling_layers(other.cnn_subsampling_layers),
    cnn_layer_types(other.cnn_layer_types),
    cnn_convolutional_layers_im2col_precomp(other.cnn_convolutional_layers_im2col_precomp),
...
void DetectionValidator::Read(std::string location) {
    ...
    if (detection_validator_stream.is_open()) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            orientations[i] =
                cv::Vec3d(orientation_tmp.at<double>(0), orientation_tmp.at<double>(1), orientation_tmp.at<double>(2));
...
float DetectionValidator::Check(
    ...
    cv::Mat_<float> ys =
        detected_landmarks_local(cv::Rect(0, detected_landmarks.rows / 2, 1, detected_landmarks.rows / 2));
    ...
    for (size_t layer = 0; layer < cnn_layer_types[view_id].size(); ++layer) {
        ...
        if (layer_type == 2) {
            ...
            fully_connected(
                outputs,
                input_maps,
                cnn_fully_connected_layers_weights[view_id][fully_connected_layer].t(),
...
void DetectionValidator::NormaliseWarpedToVector(
    const cv::Mat_<float> &warped_img,
    cv::Mat_<float> &feature_vec,
    int view_id
) {
