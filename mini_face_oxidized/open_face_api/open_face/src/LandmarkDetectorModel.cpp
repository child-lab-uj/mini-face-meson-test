...
CLNF::CLNF(const CLNF &other) :
    pdm(other.pdm), params_local(other.params_local.clone()), params_global(other.params_global),
    detected_landmarks(other.detected_landmarks.clone()), landmark_likelihoods(other.landmark_likelihoods.clone()),
    patch_experts(other.patch_experts), landmark_validator(other.landmark_validator),
    haar_face_detector_location(other.haar_face_detector_location),
    mtcnn_face_detector_location(other.mtcnn_face_detector_location), hierarchical_mapping(other.hierarchical_mapping),
    hierarchical_models(other.hierarchical_models), hierarchical_model_names(other.hierarchical_model_names),
    hierarchical_params(other.hierarchical_params), eye_model(other.eye_model),
    face_detector_MTCNN(other.face_detector_MTCNN), preference_det(other.preference_det),
    ...
    // Make sure the matrices are allocated properly
    for (std::map<int, cv::Mat_<float>>::const_iterator it = other.kde_resp_precalc.begin();
         it != other.kde_resp_precalc.end();
         it++) {
        // Make sure the matrix is copied.
        ...
        parallel_for_(cv::Range(0, hierarchical_models.size()), [&](const cv::Range &range) {
            for (int part_model = range.start; part_model < range.end; part_model++) {
                ...
                for (size_t mapping_ind = 0; mapping_ind < mappings.size(); ++mapping_ind) {
                    ...
                    part_model_locs.at<float>(mappings[mapping_ind].second + n_part_points) =
                        detected_landmarks.at<float>(mappings[mapping_ind].first + this->pdm.NumberOfPoints());
                }
                ...
                for (size_t mapping_ind = 0; mapping_ind < mappings.size(); ++mapping_ind) {
                    ...
                    detected_landmarks.at<float>(mappings[mapping_ind].first + pdm.NumberOfPoints()) =
                        hierarchical_models[part_model].detected_landmarks.at<float>(
                            mappings[mapping_ind].second + hierarchical_models[part_model].pdm.NumberOfPoints()
                        );
                }
            }
...
bool CLNF::Fit(const cv::Mat_<float> &im, const std::vector<int> &window_sizes, const FaceModelParameters &parameters) {
    ...
    // Optimise the model across a number of areas of interest (usually in descending window size and ascending scale
    // size)
    for (int scale = 0; scale < num_scales; scale++) {
        ...
        if (parameters.refine_parameters == true) {
            ...
            // Adapt the parameters based on scale (wan't to reduce regularisation as scale increases, but increase
            // sigma and Tikhonov)
            tmp_parameters.reg_factor = parameters.reg_factor -
                                        15 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);

            if (tmp_parameters.reg_factor <= 0)
                tmp_parameters.reg_factor = 0.001;

            tmp_parameters.sigma = parameters.sigma +
                                   0.25 * log(patch_experts.patch_scaling[scale_max] / 0.25) / log(2);
            tmp_parameters.weight_factor = parameters.weight_factor +
    ...
    if (parameters.weight_factor > 0) {
        ...
        for (int p = 0; p < n; p++) {
            if (!patch_experts.cen_expert_intensity.empty()) {
                ...
                for (size_t pc = 0; pc < patch_experts.svr_expert_intensity[scale][view_id][p].svr_patch_experts.size();
                     pc++) {
                    // for the x dimension
                    WeightMatrix.at<float>(p, p) =
                        WeightMatrix.at<float>(p, p) +
                        patch_experts.svr_expert_intensity[scale][view_id][p].svr_patch_experts.at(pc).confidence;
                }
                // for the y dimension
                WeightMatrix.at<float>(p + n, p + n) = WeightMatrix.at<float>(p, p);
            }
...
float CLNF::NU_RLMS(
    ...
    for (int iter = 0; iter < parameters.num_optimisation_iteration; iter++) {
        ...
        // calculate the appropriate Jacobians in 2D, even though the actual behaviour is in 3D, using small angle
        // approximation and oriented shape
        if (rigid) {
        ...
        NonVectorisedMeanShift_precalc_kde(
            mean_shifts, patch_expert_responses, dxs, dys, resp_size, a, scale, view_id, kde_resp_precalc
        );
