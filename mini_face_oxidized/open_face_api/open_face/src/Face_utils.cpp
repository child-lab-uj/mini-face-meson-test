...
void AlignFace(
    ...
    similarity_normalised_shape =
        similarity_normalised_shape(cv::Rect(0, 0, 1, 2 * similarity_normalised_shape.rows / 3)).clone();
...
void AlignFaceMask(
    ...
    // Mask each of the channels (a bit of a roundabout way, but OpenCV 3.1 in debug mode doesn't seem to be able to
    // handle a more direct way using split and merge)
...
void Extract_FHOG_descriptor(
    cv::Mat_<double> &descriptor,
    const cv::Mat &image,
    int &num_rows,
    int &num_cols,
    int cell_size
...
void ExtractSummaryStatistics(
    const cv::Mat_<double> &descriptors,
